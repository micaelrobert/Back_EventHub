    ...
    public class IngressoService : IIngressoService
    {
        ...
        public async Task<ResponseDto<IEnumerable<IngressoDto>>> GetIngressosAsync()
        {
            try
            {
                ...
                var ingressosComEventos = new List<Ingresso>();

                foreach (var ingresso in ingressos)
                {
                    var ingressoComEvento = await _ingressoRepository.GetIngressoComEventoAsync(ingresso.Id);
                    if (ingressoComEvento != null)
                        ingressosComEventos.Add(ingressoComEvento);
                }
